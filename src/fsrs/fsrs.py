        ...
        if card.state == State.Learning:
            ...
            if len(self.learning_steps) == 0 or (
                ...
                if card.last_review is None:
                    # First review: custom onboarding intervals
                    if rating == Rating.Again:
                        next_interval = timedelta(minutes=1)
                    elif rating == Rating.Hard:
                        next_interval = timedelta(minutes=15)
                    elif rating == Rating.Good:
                        next_interval = timedelta(days=1)
                    elif rating == Rating.Easy:
                        next_interval = timedelta(days=10)
                    card.state = State.Review if rating in (Rating.Good, Rating.Easy) else State.Learning
                    card.step = None if card.state == State.Review else 0
                else:
                    if rating == Rating.Again:
                        card.step = 0
